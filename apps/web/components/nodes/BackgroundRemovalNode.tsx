...
function BackgroundRemovalNodeComponent({ data }: { data: CanvasNode }) {
  return (
    <div className="w-[240px] rounded-[14px] border border-white/[0.05] bg-[#131415]/95 shadow-[0_8px_32px_rgba(0,0,0,0.35)] backdrop-blur-xl">
      ...
      <div className="flex items-center gap-2 border-b border-white/[0.04] px-3 py-2">
        <div className="flex h-6 w-6 items-center justify-center rounded-[8px] bg-[#0e2313]">
          <Scissors className="h-3 w-3 text-[#4ade80]" />
        </div>
        <span className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[#4ade80]">BG Remove</span>
        <div className="ml-auto">
          <RunStatusBadge status={data.status} />
        </div>
      </div>

      <div className="relative aspect-square overflow-hidden bg-transparent checker-bg">
        {data.previewUrl ? (
          <img src={data.previewUrl} alt={data.title} className="h-full w-full object-contain" />
        ) : (
      ...
      <div className="px-3 py-2">
        ...
        <div className="mt-0.5 text-[10px] text-[#5a6368]">Background removed</div>
      </div>
      ...
      <div className="flex items-center gap-1 border-t border-white/[0.04] px-1.5 py-1.5">
        ...
        <button className="rounded-[8px] px-2.5 py-1 text-[10px] font-medium text-[#859399] transition hover:bg-white/[0.06] hover:text-[#a4e6ff]">Edit</button>
        <button className="rounded-[8px] px-2.5 py-1 text-[10px] font-medium text-[#859399] transition hover:bg-white/[0.06] hover:text-[#a4e6ff]">3D</button>
      </div>
    </div>
  );
}
